...
> Copy the correction script `exam03_check.sh` to **server1** and run it as root from **server1**.
...
On server2, export `/srv/share/ro` read-only and `/srv/share/rw` read-write. On server1, mount both persistently in `/etc/fstab` with appropriate options.
...
## Task 19 — autofs
`🖥 server1` — autofs client (mounts exports from Task 06 on server2)
